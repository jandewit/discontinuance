...
corr_results <- cor(corr_input, method = "pearson")
...
ggplot(data_complete, aes(x = wave_re, y = mean(event))) + geom_line() + stat_smooth(method="lm", se=TRUE, fill=NA,
                formula=y ~ poly(x, 4, raw=TRUE),colour="red") + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ x, colour="blue")
...
anova(Model_bla, Model_bla2, test ="Chisq")
